...
`${name}=${value}`,
        ...
        function isFromAllowedSite(req) {
            const origin = req.headers.get('Origin') || '';
            if (origin)
                return isAllowedOriginStr(origin);
            const refOrigin = originFromRef(req);
            if (refOrigin)
                return isAllowedOriginStr(refOrigin);
            if (secFetchSameSite(req))
                return true;
            return false;
        }
        ...
        try {
            ...
            if (req.method === 'GET' && url.pathname === '/content/meta') {
                ...
                // Try returning a cached SHA first (from our in-memory stores)
                ...
                // If no cached SHA, attempt GitHub (using read token if provided)
                const headers = new Headers({
                    'Accept': 'application/vnd.github+json',
                    'User-Agent': 'monofly-anime-songs-worker'
                });
                if (env.GH_READ_TOKEN)
                    headers.set('Authorization', `Bearer ${env.GH_READ_TOKEN}`);
                ...
                try {
                    ...
                    // As a last resort, try to warm body cache by fetching full content
                    try {
                ...
                if (!res.ok) {
                    // Try to fetch full content to recover a SHA
                    try {
                        const contentRes = await fetch(api('/content'), {
                            method: 'GET',
                            headers: {
                ...
                // Successful GitHub response: parse SHA
        ...
        async function authMe(req, cors, OWNER, REPO) {
            ...
            const repo = await ghFetch(`https://api.github.com/repos/${OWNER}/${REPO}`, {
                headers
            }).then(r => r.json());
        ...
        async function getContent(cors, OWNER, REPO, BRANCH, CONTENT_PATH) {
            ...
            if (prevEtag)
                headers.set('If-None-Match', prevEtag);
        ...
        async function commitContent(req, cors, OWNER, REPO, BRANCH, CONTENT_PATH) {
            ...
            try {
                ...
                return json({
                    error: 'Failed to parse current GitHub data'
                }, 500, cors);
            }
            ...
            if (bulkUpdate) {
                ...
                if (change === null)
                    return json({
                        error: 'Cannot delete a new unsaved entry.'
                    }, 400, cors);
